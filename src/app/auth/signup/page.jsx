...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const role = userRole; // Capture the selected role for backend processing
    console.log("Selected user role for signup:", role);

    const { data: authdata, error } = await authClient.signUp.email({
      email: data.email,
      password: data.password,
      name: data.firstName,
      role: role, // Pass the selected role to the backend for user creation
      image:
        data.imgUrl ||
        "https://plus.unsplash.com/premium_photo-1781045230540-d82671bb9d53?q=80",
      callbackURL: "/login",
    });
          ...
          <div className="p-1 bg-slate-100 rounded-xl flex gap-1 mb-6">
            <button
              ...
              className={`flex-1 text-center py-2 text-xs font-semibold rounded-lg transition-all duration-150 ${
                userRole === "seeker"
                  ? "bg-slate-900 text-white shadow-sm border border-slate-200/40"
            ...
            <button
              ...
              className={`flex-1 text-center py-2 text-xs font-semibold rounded-lg transition-all duration-150 ${
                userRole === "recruiter"
                  ? "bg-blue-600 text-white shadow-sm border border-slate-200/40"
                  : "text-slate-500 hover:text-slate-900"
              }`}
            >
              Join as Recruiter
